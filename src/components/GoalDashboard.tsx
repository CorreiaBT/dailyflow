...
export function GoalDashboard() {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Card da Meta */}
      <div className="rounded-3xl border border-purple-500/30 bg-gradient-to-br from-purple-500/15 to-[#141622] p-5">
        <div className="mb-3.5 flex items-center justify-between">
          <h2 className="font-semibold text-white">🎯 Meta & Investimentos</h2>
          <button
        ...
        <div className="mb-4 flex items-center gap-3">
          <span className="rounded-2xl bg-purple-500/20 p-2.5 text-3xl">🎯</span>
          <div>
            <p className="font-bold text-white">{app.goalTitle}</p>
            <p>
              <span className="text-lg font-bold text-purple-400">{currency(app.currentSaved)}</span>{" "}
            ...
            return (
              ...
              >
                <div className="mb-1 flex items-center justify-between">
                  ...
                  {selected && <CheckCircle2 size={14} className="text-purple-400" />}
                </div>
                <p className="text-xs font-bold text-white">{info.label}</p>
                <p className="text-[11px] font-bold text-green-400">{info.annualRatePct.toFixed(1)}% a.a.</p>
              </button>
            );
          })}
        </div>
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 rounded-xl bg-black/30 p-2.5">
            ...
            <p className="font-bold text-green-400">+{currency(result.estimatedYield)}</p>
      ...
      <div className="rounded-3xl border border-white/10 bg-[#14161c] p-5">
        <h3 className="mb-3 flex items-center gap-2 font-semibold text-white">
          📈 Curva de Acúmulo & Juros (12 Meses)
        </h3>
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={chartData} margin={{ left: -20, right: 10 }}>
            ...
            />
            <Area type="monotone" dataKey="Saldo" stroke="#22c55e" strokeWidth={3} fill="url(#goalArea)" />
            <Line type="monotone" dataKey="Aportado" stroke="#a855f7" strokeWidth={2} strokeDasharray="4 4" dot={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 sm:items-center" onClick={onClose}>
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl bg-black/40 px-3 py-2.5 text-white outline-none"
        ...
        <button onClick={save} className="mt-5 w-full rounded-2xl bg-purple-600 py-3.5 font-semibold text-white">
          Salvar
        </button>
      </div>
    </div>
